...
for i = 1:Nx
    ...
    sigma(i,2) = s*(1/9)*sqrt(ujump(i)^2 + ujump(i + 1)^2)/hx;
    sigma(i,2) = sigma(i,2) + s*(1/3)*sqrt(uxjump(i)^2 + uxjump(i + 1)^2)/hx;

    sigma(i,3) = s*(4/135)*sqrt(ujump(i)^2 + ujump(i + 1)^2)/hx;
    sigma(i,3) = sigma(i,3) + s*(4/45)*sqrt(uxjump(i)^2 + uxjump(i + 1)^2)/hx;
    sigma(i,3) = sigma(i,3) + s*(2/27)*sqrt(uxxjump(i)^2 + uxxjump(i + 1)^2)/hx;
end
